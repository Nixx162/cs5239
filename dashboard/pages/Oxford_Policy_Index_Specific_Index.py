...
index_explanations = {
    ...
    'E3_Fiscal measures': 'Announced economic stimulus spending (excluding income support and debt relief) in response to COVID-19.',
    'E4_International support': 'Announced offers of COVID-19 related aid spending to other countries.',
    ...
    'H4_Emergency investment in healthcare': 'Announced short-term spending on healthcare system, e.g., hospitals, masks, etc.',
    'H5_Investment in vaccines': 'Announced public spending on vaccine development.',
...
for index in index_columns:
    us_df[index] = us_raw[index].fillna(0)
...
us_df_grouped = us_df.groupby('Date').agg(aggregation_dict).reset_index()
...
for index in index_columns:
    can_df[index] = can_raw[index].fillna(0)
can_df['Date'] = pd.to_datetime(can_df['Date'], format='%Y%m%d')

# Group by Date and calculate mean for indexes, sum for cases and deaths
can_df_grouped = can_df.groupby('Date').agg(aggregation_dict).reset_index()
...
st.header("Policy Indexes")
# ------------------ First Plot ------------------
# Daily case rate with selectable index using selectbox
st.text("Select an Index to Display with Daily Case Rate")
...
if selected_index:
    ...
    fig_cases_indexes.update_layout(
        title_text="Daily COVID-19 Case Rate and Selected Index Over Time",
        legend=dict(
            orientation="h",
            yanchor="bottom",
            y=-0.4,
...
if selected_index_death:
    ...
    fig_deaths_indexes.update_layout(
        title_text="Daily COVID-19 Death Rate and Selected Index Over Time",
        legend=dict(
            orientation="h",
            yanchor="bottom",
            y=-0.4,
